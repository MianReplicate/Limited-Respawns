...
public class LRUtil {
    ...
    public static boolean saveLRDataToProfile(MinecraftServer server, GameProfile gameProfile, Function<IDataHolder, CompoundTag> function){
        try {
            ...
            if(playerTag != null && dataHolder != null){
                LevelStorageSource.LevelStorageAccess levelStorageAccess = server.storageSource;
                File playerDir = levelStorageAccess.getLevelPath(LevelResource.PLAYER_DATA_DIR).toFile();
                String uuidString = gameProfile.getId().toString();
                CompoundTag compoundTag = function.apply(dataHolder);
